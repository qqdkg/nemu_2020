...
static inline def_rtl(push, const rtlreg_t* src1) {
  ...
  // M[esp] <- src1
  // TODO();
  rtl_subi(s, &reg_l(R_ESP), &reg_l(R_ESP), sizeof(rtlreg_t));
  rtl_sm(s, &reg_l(R_ESP), 0, src1, sizeof(rtlreg_t));
}
...
static inline def_rtl(pop, rtlreg_t* dest) {
  ...
  // esp <- esp + 4
  // TODO();
  rtl_lm(s, dest, &reg_l(R_ESP), 0, sizeof(rtlreg_t));
  rtl_addi(s, &reg_l(R_ESP), &reg_l(R_ESP), sizeof(rtlreg_t));
}
